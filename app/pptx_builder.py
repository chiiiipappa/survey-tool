...
def _add_slide(prs: Presentation, slide, page: dict, cr: dict) -> None:
    ...
    show_table: bool = lc.get("showTable", False)
    ...
    if is_table_only:
        ...
        table_h = CONTENT_H * 0.40
        table_top = CONTENT_TOP + chart_h + Inches(0.07)
        _add_chart_area(slide, cr, cc, mode, MARGIN_L, CONTENT_TOP, CONTENT_W, chart_h)
        _add_table_shape(slide, cr, lc, cc, MARGIN_L, table_top, CONTENT_W, table_h)
    else:
...
def _add_table_shape(slide, cr: dict, lc: dict, cc: dict, x, y, w, h) -> None:
    ...
    n_rows_total = n_header_rows + n_n_rows + n_data_rows
...
def _resolve_colors(cc: dict, labels: list[str]) -> list[str]:
    ...
    value_color_map: dict = cs.get("valueColorMapping") or {}
    ...
    for label in labels:
        ...
        elif label in value_color_map:
            colors.append(value_color_map[label])
        elif label in overridden:
